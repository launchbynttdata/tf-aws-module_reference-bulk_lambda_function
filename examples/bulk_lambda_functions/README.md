...
| <a name="input_lambda_function_name_overrides"></a> [lambda\_function\_name\_overrides](#input\_lambda\_function\_name\_overrides) | Optional map of Lambda function keys to custom full names. When set, overrides the resource\_names-generated name for that function. | `map(string)` | `{}` | no |
...
| <a name="output_lambda_function_name_overrides"></a> [lambda\_function\_name\_overrides](#output\_lambda\_function\_name\_overrides) | Exposed for tests: when set, deployed Lambda names should match these overrides. |
