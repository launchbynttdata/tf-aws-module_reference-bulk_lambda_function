...
| <a name="input_lambda_function_name_overrides"></a> [lambda\_function\_name\_overrides](#input\_lambda\_function\_name\_overrides) | Optional map of Lambda function keys (matching bulk_lambda_functions keys) to custom full names. When a key is present, its value is used as the Lambda function name instead of the resource_names-generated name. | `map(string)` | `{}` | no |
